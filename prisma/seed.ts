// ─────────────────────────────────────────────────────────────
//  Civilic seed — پروژه واقعی «پارکینگ شرقی خاتم»
//  مبنا: تحلیل فایل واقعی Important project.svzt
//  سه طرف واقعی + کاربران نقش‌دار + نمونه‌های گردش‌کار صورت‌وضعیت/چت/مکاتبات/اسناد/تعدیل
...
const PWD = hashPassword("civilic"); // رمز همه‌ی کاربران نمونه

async function main() {
  console.log("🚀 شروع seed — پروژه پارکینگ شرقی خاتم...");
  ...
  const tenant = await db.tenant.create({
    data: { name: "Civilic", signatures: "[]" },
  });

  // ── سازمان‌ها (سه طرف واقعی) ──
  const employerOrg = await db.organization.create({
    data: { tenantId: tenant.id, name: "دانشگاه خاتم", type: "EMPLOYER", address: "تهران، پردیس دانشگاه خاتم" },
  });
  const consultantOrg = await db.organization.create({
    data: { tenantId: tenant.id, name: "مهندسین مشاور شارستان", type: "CONSULTANT" },
  });
  const contractorOrg = await db.organization.create({
    data: { tenantId: tenant.id, name: "شرکت سیوان تدبیر تجارت", type: "CONTRACTOR" },
  });
  ...
  const priceList = await db.priceList.create({
    data: { year: 1403, discipline: "ابنیه", title: "فهرست‌بهای ابنیه ۱۴۰۳" },
  });
  ...
  const project = await db.project.create({
    data: {
      ...
      description: "سازه‌ی پارکینگ شرقی مجموعه آموزشی و فناوری خاتم — وارد شده از تکسا و ادامه‌یافته در Civilic.",
      recordSource: "TEXSA",
  ...
  // ── طرفین پروژه ──
  const employerParty = await db.projectParty.create({
    data: { projectId: project.id, organizationId: employerOrg.id, partyType: "EMPLOYER", displayTitle: "کارفرما — دانشگاه خاتم", isPrimary: true },
  });
  const consultantParty = await db.projectParty.create({
    data: { projectId: project.id, organizationId: consultantOrg.id, partyType: "CONSULTANT", displayTitle: "مشاور — مهندسین مشاور شارستان", isPrimary: true },
  });
  const contractorParty = await db.projectParty.create({
    data: { projectId: project.id, organizationId: contractorOrg.id, partyType: "CONTRACTOR", displayTitle: "پیمانکار — سیوان تدبیر تجارت", isPrimary: true },
  });

  // ── اعضای پروژه با نقش واقعی ──
  async function mkMember(userId: string, partyId: string, role: string, canApprove = false, canSign = false) {
    return db.projectMember.create({
      data: { projectId: project.id, userId, projectPartyId: partyId, role, canApprove, canSign, isActive: true },
    });
  }
  ...
  // ── صورت‌وضعیت‌ها با حالت‌های مختلف گردش‌کار ──
  ...
  for (const p of payStates) {
    ...
    const pay = await db.payment.create({
      data: {
        ...
        insurance,
        tax,
        netPayable: p.amount - guarantee - insurance - tax,
        recordSource: "TEXSA",
  ...
  // ── تعدیل مرتبط با صورت‌وضعیت تأییدشده (دوره ۲) ──
  await db.adjustmentReportRow.create({
    data: {
      tenantId: tenant.id,
      projectId: project.id,
      paymentId: payments[1].id,
      periodLabel: "سه‌ماهه دوم ۱۴۰۳",
      chapterNo: 6,
      workPeriodAmount: 60822769244,
      baseIndex: 6970.7,
      currentIndex: 8120.4,
      adjustmentFactor: 1.165,
      adjustmentAmount: 60822769244 * 0.165,
      adjustmentType: "TEMPORARY",
      recordSource: "TEXSA",
      notes: "تعدیل بر اساس شاخص ابنیه سه‌ماهه دوم ۱۴۰۳.",
    },
  });

  // ── کانال‌های پروژه (پیش‌فرض) ──
  ...
  // کانال خودکار برای هر صورت‌وضعیت
  for (const p of payStates) {
    const ch = await db.projectChannel.create({
      data: { projectId: project.id, title: `صورت‌وضعیت شماره ${p.period}`, type: "PAYMENT", visibility: "ALL_PARTIES", entityType: "PAYMENT", entityId: payments[p.period - 1].id },
    });
    await db.message.create({
      data: { channelId: ch.id, senderId: "system", senderName: "سامانه", systemType: "SUBMIT", entityType: "PAYMENT", entityId: payments[p.period - 1].id, body: `کانال صورت‌وضعیت شماره ${p.period} ایجاد شد. وضعیت فعلی: ${p.status}` },
    });
  }
  ...
  const letter = await db.letter.create({
    data: {
      projectId: project.id,
      fromPartyId: consultantParty.id,
      toPartyId: contractorParty.id,
      subject: "درخواست اصلاح صورت‌وضعیت شماره ۳",
      body: "با سلام، احجام فصل ۶ صورت‌وضعیت شماره ۳ نیازمند صورت‌جلسه‌ی تأییدشده است. لطفاً اصلاح و مجدداً ارسال فرمایید.",
      letterNo: "SH-1403-118",
      letterDate: new Date("2024-09-10"),
      status: "SENT",
    },
  });
  ...
  const doc = await db.document.create({
    data: { projectId: project.id, title: "صورت‌جلسه احجام فصل ۶", type: "MINUTES", entityType: "PAYMENT", entityId: payments[2].id },
  });
  const ver = await db.documentVersion.create({
    data: { documentId: doc.id, versionNo: 1, fileUrl: "/uploads/demo/minutes-ch6-v1.pdf", fileName: "minutes-ch6-v1.pdf", fileSizeBytes: 102400, uploadedById: conResident.id, uploadedByName: conResident.name },
  });
  await db.document.update({ where: { id: doc.id }, data: { currentVersionId: ver.id } });

  // ── هشدارها ──
  await db.alert.create({
    data: { tenantId: tenant.id, projectId: project.id, type: "WORKFLOW", severity: "WARNING", title: "صورت‌وضعیت برگشتی", message: "صورت‌وضعیت شماره ۳ توسط مشاور برگشت داده شده و منتظر اصلاح پیمانکار است.", relatedType: "PAYMENT", relatedId: payments[2].id },
  });
  await db.alert.create({
    data: { tenantId: tenant.id, projectId: project.id, type: "WORKFLOW", severity: "INFO", title: "منتظر تأیید کارفرما", message: "صورت‌وضعیت شماره ۲ تأیید مشاور را گرفته و منتظر بررسی کارفرماست.", relatedType: "PAYMENT", relatedId: payments[1].id },
  });

  console.log("✅ seed کامل شد: ۱ پروژه، ۳ طرف، ۱۲ کاربر، ۵ صورت‌وضعیت، ۱۴ کانال، مکاتبه و سند و تعدیل نمونه.");
  console.log("🔑 رمز همه‌ی کاربران: civilic");
}
