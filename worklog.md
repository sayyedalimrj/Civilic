...
## Remaining Core Product Completion Phase
Agent: Kiro (Claude Opus 4.8). مبنا: روی شاخه‌ی PR #4 (آپلود/پیوست امن). راستی‌آزمایی: `prisma generate` ✅، `tsc --noEmit` ۰ خطا، `eslint .` بدون خطا، `next build` موفق (۹ route جدید ثبت شدند).

### قبلاً انجام‌شده (PR #4 و پیش از آن)
- زیرساخت آپلود/پیوست امن: مدل `Attachment`, `upload-security`, `/api/uploads*`, کامپوننت‌های `SecureFileUpload/AttachmentList/AttachmentsPanel/PermissionGate`, اتصال به جزئیات صورت‌وضعیت.
- مدل داده‌ی تقریباً کامل (Organization/ProjectParty/ProjectMember/Tenant/Review tables/TexsaImport/RoundTripReport/CalculationNode).
- کتابخانه‌های تکسا: parse/analyze/preserve-raw/import-local/normalize/export/roundtrip.
- کامپوننت‌های رسیدگی redline (`payment/adjustment/measurement review`, `review-value-cell`, `strike-through-value`, `redline-review-panel`).

### چه چیزی باقی مانده بود (و در این فاز انجام شد)
1. **سند ممیزی شکاف**: `docs/remaining-core-product-gap-audit.md` (وضعیت DONE/PARTIAL/MISSING هر بخش + فایل‌ها + کار باقی‌مانده + اولویت).
2. **APIهای مدیریت سازمان/طرف/عضو پروژه** (نبودند):
   - `GET/POST /api/organizations` — فهرست/ساخت سازمان مستاجر؛ ساخت فقط برای مدیر سامانه یا مدیر/مالک مستاجر (`canManageTenant`).
   - `GET/POST /api/projects/[id]/parties` — افزودن طرف (هر ۷ نوع: EMPLOYER/CONSULTANT/CONTRACTOR/LAB/SUPPLIER/OPERATOR/OTHER) با مجوز `members.edit`.
   - `GET/POST /api/projects/[id]/members` — افزودن کاربر به طرف با نقش پروژه‌ای (اعتبارسنجی نقش با `ROLE_PERMISSIONS`، مرز مستاجر روی کاربر هدف، یکتایی عضویت) با مجوز `members.invite`.
   - `PATCH/DELETE /api/projects/[id]/members/[memberId]` — ویرایش نقش/امضا/فعال‌بودن و حذف عضو (از دست رفتن فوری دسترسی) با `members.edit`/`members.disable`.
   - گسترش `POST /api/projects`: انواع طرف کامل شد، **سازنده** به‌صورت خودکار به‌عنوان عضو (`employer_project_manager`) افزوده می‌شود، و audit ساخت پروژه ثبت می‌شود.
3. **route‌های ویزارد ایمپورت تکسا** (کتابخانه‌ها بودند، route نبود):
   - `POST /api/texsa/imports/from-server-file` + `GET` (فهرست `.svzt` داخل `TEXSA_UPLOAD_DIR`) — ایمپورت از فایل سرور با **مهار path traversal** و `importLocalSvzt`.
   - `POST /api/texsa/imports/[id]/normalize` — ساخت/به‌روزرسانی پروژه‌ی Civilic با `normalizeImport` و ذخیره‌ی `projectId` روی import.
   - `GET /api/texsa/imports/[id]/progress` — وضعیت/تعداد ردیف‌های ذخیره‌شده/normalize‌شده.
   - `GET /api/texsa/imports/[id]/roundtrip-report` — گزارش سازگاری round-trip (`generateRoundTripReport`).
   - کنترل دسترسی: `src/lib/auth/texsa-access.ts` (`canUseTexsa` = دسترسی پلتفرم یا عضویت پروژه‌ای با مجوز `texsa.import`/`texsa.export`).
4. **اتصال پیوست‌ها به ماژول‌های بیشتر** (فراتر از صورت‌وضعیت):
   - تعدیل (`adjustment-review-view`, ownerType=ADJUSTMENT)، متره (`measurement-review-view`, ownerType=MEASUREMENT)، اسناد برگه مالی (`financial-sheet-view`, ownerType=DOCUMENT) — همگی با `AttachmentsPanel` + `useProjectAccess` و gating مجوز.

### فایل‌های تغییر/افزوده‌شده
- جدید: `docs/remaining-core-product-gap-audit.md`, `src/lib/auth/tenant-access.ts`, `src/lib/auth/texsa-access.ts`, `src/app/api/organizations/route.ts`, `src/app/api/projects/[id]/parties/route.ts`, `src/app/api/projects/[id]/members/route.ts`, `src/app/api/projects/[id]/members/[memberId]/route.ts`, `src/app/api/texsa/imports/from-server-file/route.ts`, `src/app/api/texsa/imports/[id]/normalize/route.ts`, `src/app/api/texsa/imports/[id]/progress/route.ts`, `src/app/api/texsa/imports/[id]/roundtrip-report/route.ts`.
- ویرایش: `src/app/api/projects/route.ts`, `src/components/review/adjustment-review-view.tsx`, `src/components/review/measurement-review-view.tsx`, `src/components/views/project/financial-sheet-view.tsx`.

### نحوه‌ی تست
- سازمان: `POST /api/organizations {name,type}` با کاربر مدیر مستاجر → ۲۰۱؛ با کاربر عادی → ۴۰۳.
- عضو پروژه: `POST /api/projects/{id}/members {userId,projectPartyId,role}` با نقش دارای `members.invite`؛ نقش نامعتبر → ۴۰۰؛ کاربر مستاجر دیگر → ۴۰۰؛ تکراری → ۴۰۹.
- حذف عضو: `DELETE …/members/{memberId}` → دسترسی فوری قطع می‌شود.
- تکسا server-file: فایل `.svzt` را در `TEXSA_UPLOAD_DIR` قرار دهید → `GET /api/texsa/imports/from-server-file` فهرست می‌دهد → `POST {fileName}` ایمپورت می‌کند → `POST …/[id]/normalize` پروژه می‌سازد → `GET …/[id]/roundtrip-report` گزارش می‌دهد.
- پیوست: در تعدیل/متره/برگه مالی فایل آپلود/دانلود/حذف کنید (همه permission-checked).

### یادداشت‌های امنیتی
- همه‌ی route‌های جدید: `getCurrentUser` + مجوز سمت‌سرور + مرز مستاجر + audit. هیچ سرو عمومی فایل.
- ایمپورت از فایل سرور فقط `basename` داخل `TEXSA_UPLOAD_DIR` (بدون path دلخواه).
- نقش پروژه‌ای است؛ کاربر هدف باید هم‌مستاجر باشد؛ نقش‌ها با `ROLE_PERMISSIONS` اعتبارسنجی می‌شوند.

### محدودیت‌ها / فاز بعد (شفاف)
- **UI**: این فاز عمدتاً API + اتصال پیوست بود. UIِ گام‌به‌گامِ مدیریت عضو/سازمان و ویزارد ۶-مرحله‌ای ساخت پروژه و ویزارد UI ایمپورت تکسا و بازطراحی کامل workspace پروژه (rail توالی + side panel یکپارچه + کاهش تب) هنوز نیاز به کار دارند.
