...
import { addMeasurementItemReview } from "@/lib/review/service";
import type { ReviewParty, ReviewDecision } from "@/lib/review/layers";

// POST — افزودن لایه‌ی رسیدگی یک ردیف ریزمتره
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string; itemId: string }> }) {
  ...
  const body = (await req.json().catch(() => ({}))) as { decision?: ReviewDecision; quantity?: number; comment?: string };
  ...
  await addMeasurementItemReview({ itemId, partyType: party, decision, quantity: body.quantity ?? null, comment: body.comment ?? null, userId: user.id, userName: user.name });
